...
exports.batman_create_Page = function (req, res) {
    console.log("create view")
    try {
        res.render('batmancreate', { title: 'batman Create' });
    }
    catch (err) {
        res.status(500)
        res.send(`{'error': '${err}'}`);
...
exports.batman_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
        let result = await batman.findById(req.query.id)
        res.render('batmanupdate', { title: 'batman Update', toShow: result });
    }
    catch(err){
        res.status(500)
        res.send(`{'error': '${err}'}`);
   ...
   exports.batman_delete_Page = async function(req, res) { 
    console.log("Delete view for id "  + req.query.id) 
    try{ 
        result = await batman.findById(req.query.id) 
        res.render('batmandelete', { 
            title: 'batman Delete', toShow: 
                result 
        }); 
    } 
    catch(err) { 
